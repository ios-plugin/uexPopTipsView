...
#define kDefaultSelectedTextColor [UIColor whiteColor]
...
- (void)presentPopoverFromRect:(CGRect)rect inView:(UIView *)view withStrings:(NSArray *)stringArray selectedIndex:(NSInteger)selectedIndex {
    ...
    for (NSString *string in stringArray) {
        ...
        if ([[UIDevice currentDevice].systemVersion floatValue] >7.0) {
            ...
            textSize = [string sizeWithFont:kTextFont];
        }
...
//¡™£¢∞§¶•ªº–«åß∂∂ƒ©˙∆˚¬…æΩ≈ç√∫µ≤≥÷ÒŒ„´‰ˇÁ¨ˆØ∏”’⁄€‹›ﬁﬂ‡°·‚—±»Å¸ÍÎÏ˝ÓÔÒÚÆ¸˛Ç◊ı˜Â¯˘¿1236589+.000
...
- (UIButton *)getControlButton:(BOOL)rightArrow {
    ...
    if (rightArrow) {
        ...
        if ([[UIDevice currentDevice].systemVersion floatValue]>6.0) {
            ...
            [res setTitle:@"》" forState:UIControlStateNormal];
        }
...
//            [self.popOverDividerColor setFill];
            ...
            rect1.origin.x += self.contentView.frame.origin.x +0.6;
            rect1.origin.y += self.contentView.frame.origin.y;
            rect1.size.width = 0.4;
            UIBezierPath *dividerPath1 = [UIBezierPath bezierPathWithRect:rect1];
            [self.popOverDividerColor setFill];
            [dividerPath1 fill];
        }
    }
...
//        return kDefaultBackgroundColor;
        return [UIColor colorWithRed:23/255.0 green:29/255.0 blue:30/255.0 alpha:0.9];
    }
